    ...
    private void drawData(Canvas canvas) {
        for (int i = 0; i < mRadarData.size(); i++) {
            ...
            for (int j = 1; j <= values.size(); j++) {
                ...
                float x = (float) (mPointCenter.x
                        + Math.sin(mAngle * j + mRotateAngle) * mRadius * percent);
                float y = (float) (mPointCenter.y
                        + Math.cos(mAngle * j + mRotateAngle) * mRadius * percent);
                if (j == 1) {
                    mRadarPath.moveTo(x, y);
                } else {
                    mRadarPath.lineTo(x, y);
            ...
            if (radarData.isValueTextEnable()) {
                ...
                for (int k = 0; k < textPoint.length; k++) {
                    ...
                    float             textHeight  = fontMetrics.descent - fontMetrics.ascent;
                    canvas.drawText(text, textPoint[k].x - textWidth / 2,
                                    textPoint[k].y + textHeight / 3, mValueTextPaint);
                }
            }
        }
    }
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
            if (Math.abs(velocityX) > Math.abs(velocityY)) {
                mFlingPoint = e2.getX();
                mScroller.fling((int) e2.getX(), 0, (int) velocityX, 0,
                                (int) (-mPerimeter + e2.getX()), (int) (mPerimeter + e2.getX()), 0,
                                0);
            } else if (Math.abs(velocityY) > Math.abs(velocityX)) {
                mFlingPoint = e2.getY();
                mScroller.fling(0, (int) e2.getY(), 0, (int) velocityY, 0, 0,
                                (int) (-mPerimeter + e2.getY()), (int) (mPerimeter + e2.getY()));
            }
            invalidate();
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            ...
            double dis = RotateUtil.getRotateAngle(
                    new PointF(e2.getX() - distanceX, e2.getY() - distanceY),
