...
public class GoogleCircleProgressView extends ImageView {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr) {
		final TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.GoogleCircleProgressView,
				defStyleAttr, 0);
		final float density = getContext().getResources().getDisplayMetrics().density;

		mBackGroundColor = a.getColor(R.styleable.GoogleCircleProgressView_gcp_background_color,
				DEFAULT_CIRCLE_BG_LIGHT);

		mProgressColor = a.getColor(R.styleable.GoogleCircleProgressView_gcp_progress_color,
				DEFAULT_CIRCLE_BG_LIGHT);
		...
		mTextSize = a.getDimensionPixelOffset(R.styleable.GoogleCircleProgressView_gcp_progress_text_size,
		...
		mCircleBackgroundEnabled = a.getBoolean(
				R.styleable.GoogleCircleProgressView_gcp_enable_circle_background, true);
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (!elevationSupported()) {
			setMeasuredDimension(getMeasuredWidth() + mShadowRadius * 2, getMeasuredHeight() + mShadowRadius
					* 2);
		}
	}
