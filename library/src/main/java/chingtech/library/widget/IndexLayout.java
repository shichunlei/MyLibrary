...
public class IndexLayout extends FrameLayout {
    ...
    private int   indexBarWidth       = (int) dp2px(30);
    ...
    public IndexLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        indexBar.setIndexTextSize(20);
        indexBar.setSelTextColor(Color.BLUE);
        indexBar.setNorTextColor(Color.GREEN);
        addView(indexBar);
    }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        childView.layout(getWidth() - indexBarWidth, (int) (getHeight() * topPadding), getWidth(),
                         (int) (getHeight() * (topPadding + indexBarHeightRatio)));//放在最右边
    }
        ...
        if (isShowCircle) {
            ...
            if (isDrawByTouch) {
                canvas.drawCircle(getWidth() / 2,
                                  touchYpivot + getHeight() * (1 - indexBarHeightRatio) / 2,
                                  circleRadius, mPaint);
                canvas.drawText(indexName, getWidth() / 2,
                                touchYpivot + yAxis + getHeight() * (1 - indexBarHeightRatio) / 2,
                                mTxtPaint);
            } else {
