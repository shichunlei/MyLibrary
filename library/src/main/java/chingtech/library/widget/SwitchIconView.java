...
public class SwitchIconView extends AppCompatImageView {
    ...
    public SwitchIconView(@NonNull Context context, @Nullable AttributeSet attrs,
        ...
        TypedArray array = getContext().getTheme()
                                       .obtainStyledAttributes(attrs, R.styleable.SwitchIconView, 0,
                                                               0);

        try {
            iconTintColor = array.getColor(R.styleable.SwitchIconView_siv_tint_color, Color.BLACK);
            animationDuration = array.getInteger(R.styleable.SwitchIconView_siv_animation_duration,
                                                 DEFAULT_ANIMATION_DURATION);
            disabledStateAlpha = array.getFloat(R.styleable.SwitchIconView_siv_disabled_alpha,
                                                DEFAULT_DISABLED_ALPHA);
            disabledStateColor = array.getColor(R.styleable.SwitchIconView_siv_disabled_color,
                                                iconTintColor);
            enabled = array.getBoolean(R.styleable.SwitchIconView_siv_enabled, true);
            noDash = array.getBoolean(R.styleable.SwitchIconView_siv_no_dash, false);
        } finally {
            array.recycle();
        }

        if (disabledStateAlpha < 0f || disabledStateAlpha > 1f) {
            throw new IllegalArgumentException("Wrong value for si_disabled_alpha ["
                                                       + disabledStateAlpha
                                                       + "]. "
    ...
    protected void onSizeChanged(int width, int height, int oldWidth, int oldHeight) {
        ...
        dashThickness = (int) (DASH_THICKNESS_PART * (dashLengthXProjection + dashLengthYProjection)
                / 2f);
    ...
    private void updateClipPath() {
        ...
        clipPath.lineTo(dashXStart + dashLengthXProjection * fraction,
                        dashYStart + dashLengthYProjection * fraction - delta);
        clipPath.lineTo(dashXStart + dashLengthXProjection * fraction - delta,
    ...
    private void updateColor(float fraction) {
        if (iconTintColor != disabledStateColor) {
            final int color = (int) colorEvaluator.evaluate(fraction, iconTintColor,
                                                            disabledStateColor);
    ...
    private void updateAlpha(float fraction) {
        int alpha = (int) ((disabledStateAlpha + (1f - fraction) * (1f - disabledStateAlpha))
                * 255);
