...
public class DashboardView extends View {
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        centerCircleRadius = a.getDimensionPixelSize(R.styleable.DashboardView_centerCircleRadius,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (oval == null) {
            oval = new RectF(x + (px - py), maxRadius / 10, maxRadius * 2 - x + (px - py),
                             maxRadius * 2 - x);
        }
        if (px > py) {
            px = (px - py) + maxRadius;
        ...
        for (int i = 0; i < intervals.length; i++) {
            mArcPaint.setColor(ContextCompat.getColor(getContext(),
                                                      i < colorRes.length ? colorRes[i] :
                                                              colorRes[0]));
        ...
        canvas.drawText(text, px - (text.length() > 3 ? mTextHeight * 3 : mTextHeight),
                        2 * maxRadius - mTextHeight + 6, mNumberPaint);
        canvas.save();
        ...
        for (int i = startNum; i <= totalScores + startNum; i++) {
            ...
            if (i % scaleValInv == 0) {
                ...
                if (isDraw) {
                    canvas.drawText(i + "", px - mTextHeight + 6, graduatedLineY
                            + scaleValueinv
                            + graduatedLineLength
                            + graduatedLineinv
            ...
            if (i % graduatedInv == 0) {
                canvas.drawLine(px, graduatedLineY, px, graduatedLineY + graduatedLineLength,
                                mLinePaint);
            }
            if (i == angle) {
                canvas.drawLine(px, py - centerCircleRadius, px,
                                (int) (py - maxRadius * linePercent), mCenterCirclePaint);
            }
            if (intervalStr != null && invPos < intervals.length) {
                if (i - 1 == inv + intervals[invPos] / 2) {
                    ...
                    canvas.drawTextOnPath(intervalStr[invPos], path, 0,
    ...
    public void initDash(int startNum, int[] eachInterval, String[] intervalStrs, String unit,
            int[] colors) {
