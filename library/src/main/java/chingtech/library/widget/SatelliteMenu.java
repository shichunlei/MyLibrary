 ...
 * Description:
 * Created by
 * Created at 17/10/28 上午10:37
 *
 * 原项目地址：https://github.com/chenshouyin/SatelliteMenu
 */
public class SatelliteMenu extends RelativeLayout implements View.OnClickListener {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mMenuImageWidth = a.getDimension(R.styleable.SatelliteMenu_menu_image_width,
                                         45);//get dp will be convert to px
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        super.onMeasure(widthMeasureSpec,
                        heightMeasureSpec);//默认只有这一句 这个只是父类的measure所以getMeasuredWidth()是有值的
    }
    ...
    private void animMenuItem() {
        ...
        for (int i = 0; i < textViews.size(); i++) {
            ...
            float          mY        = 0;
            if (mPostion == MenuPosition.LEFT_TOP) {
                mX = ((float) (distance * Math.sin(Math.PI / 2 / (textViews.size() - 1) * i)))
                        * flag;
                mY = ((float) (distance * Math.cos(Math.PI / 2 / (textViews.size() - 1) * i)))
                        * flag;
            } else if (mPostion == MenuPosition.RIGHT_TOP) {
                mX = (float) (-distance * Math.sin(Math.PI / 2 / (textViews.size() - 1) * i))
            ...
            animatorX.addListener(new Animator.AnimatorListener() {
                ...
                public void onAnimationEnd(Animator animator) {
                    if (!isMenuOpen) {
                        ...
                        ObjectAnimator animatorAlpha = ObjectAnimator.ofFloat(imageView, "alpha",
                                                                              0f, 1f);
    ...
    public void setMenuItemImage(List<Integer> imageResource) {
        ...
        for (int i = 0; i < imageResource.size(); i++) {//Dynamic add view
            ...
            RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams(
                    (int) mMenuItemImageWidth, (int) mMenuItemImageWidth);
            ...
            if (nameMenuItem == null) {//不设置文字
                ...
                drawable.setBounds(0, 0, (int) mMenuItemImageWidth - Math.max(rect.width() * 2,
                                                                              rect.height()),
    ...
    public void setMenuImage(int imageResource) {
        RelativeLayout.LayoutParams params = new RelativeLayout.LayoutParams((int) mMenuImageWidth,
                                                                             (int) mMenuImageWidth);
    ...
    public class MenuPosition {
        public static final int LEFT_TOP     = 0;//主菜单在右下角
        public static final int RIGHT_TOP    = 1;//主菜单在左下角
        public static final int LEFT_BOTTOM  = 2;//主菜单在右上角
        public static final int RIGHT_BOTTOM = 3;//主菜单在左上角
    }
}
