...
public class DescProgressView extends View {
    ...
    private void initAttrs(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        TypedArray typedArray = context.getTheme()
                                       .obtainStyledAttributes(attrs, R.styleable.DescProgressView,
                                                               defStyleAttr,
                                                               R.style.Def_DescProgressViewStyle);

        textNormalColor = typedArray.getColor(R.styleable.DescProgressView_dpv_text_normal_color,
                                              Color.BLACK);
        textSelectedColor = typedArray.getColor(R.styleable.DescProgressView_dpv_text_seleced_color,
                                                Color.BLACK);
        dpvTextSize = typedArray.getDimensionPixelSize(R.styleable.DescProgressView_dpv_text_size,
                                                       0);
        dpvProgressBgColor = typedArray.getColor(R.styleable.DescProgressView_dev_progress_bg_color,
                                                 Color.BLACK);
        dpvSmallCicleColor = typedArray.getColor(
                R.styleable.DescProgressView_dev_progress_small_circle_color, Color.BLACK);
        dpvBigCircleColor = typedArray.getColor(
                R.styleable.DescProgressView_dev_progress_big_circle_color, Color.BLACK);
        typedArray.recycle();
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (widthMode == MeasureSpec.AT_MOST || widthMode == MeasureSpec.UNSPECIFIED) {
            widthSize = (int) dp2px(screenWidth);
            widthMeasureSpec = MeasureSpec.makeMeasureSpec(widthSize, MeasureSpec.EXACTLY);
        }

        if (heightMode == MeasureSpec.AT_MOST || heightMode == MeasureSpec.UNSPECIFIED) {
            heightSize = (int) dp2px(DEF_VIEW_HEIGHT);
    ...
    private void getColorFullRectF() {
        float bTop = bgLineRect.top;
        float bLeft = bgLineRect.left + (bgCirclePoints.get(currentSelectPosition - 2).x
                - bgCirclePoints.get(0).x);
        float bBottom = bgLineRect.bottom;
        float bRight = bLeft + (bgCirclePoints.get(currentSelectPosition - 1).x
                - bgCirclePoints.get(currentSelectPosition - 2).x);
    ...
    private void getSelectedRectF() {
        ...
        float bBottom = bgLineRect.bottom;
        float bRight = bgLineRect.left + (bgCirclePoints.get(currentSelectPosition - 2).x
                - bgCirclePoints.get(0).x);
    ...
    private void getBgLineRectF() {
        ...
        int bRight = dpViewWidth
                - allDescTextPoints.get(allDescTextPoints.size() - 1).x / 2
                - leftAndRightPadding;
    ...
    private void drawDescText(Canvas canvas) {
        if (descs != null && descs.size() > 1) {
            for (int i = 0; i < descs.size(); i++) {
                ...
                canvas.drawText(descs.get(i), currentTextPoint.x, currentTextPoint.y,
                                textDescPaint);
            }
        }
    }
    ...
    private void drawSelectedLine(Canvas canvas) {
        ...
        LinearGradient linearGradient = new LinearGradient(gradualRectF.left, gradualRectF.top,
                                                           gradualRectF.right, gradualRectF.bottom,
                                                           dpvSmallCicleColor, dpvBigCircleColor,
