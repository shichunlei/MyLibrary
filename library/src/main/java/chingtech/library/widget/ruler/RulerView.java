 ...
 * Description:
 * Created by
 * Created at 17/10/28 上午10:01
 *
 * 原项目地址：https://github.com/superSp/RulerView
 */
public class RulerView extends View {
    ...
    private void autoVelocityScroll(int xVelocity) {
        ...
        valueAnimator = ValueAnimator.ofInt(0, xVelocity / 20)
                                     .setDuration(Math.abs(xVelocity / 10));
    ...
    private void drawScaleAndNum(Canvas canvas) {
        canvas.translate(0, (showScaleResult ? resultNumRect.height() : 0)
                + rulerToResultgap);//移动画布到结果值的下面
        ...
        canvas.translate(num2,
                         0);    //不加该偏移的话，滑动时刻度不会落在0~1之间只会落在整数上面,其实这个都能设置一种模式了，毕竟初衷就是指针不会落在小数上面
        //这里是滑动时候不断回调给使用者的结果值
        resultText = String.valueOf(new WeakReference<>(
                new BigDecimal((width / 2 - moveX) / (scaleGap * scaleCount))).get()
                                                                              .setScale(1,
        ...
        while (rulerRight < width) {
            if (num1 % scaleCount == 0) {
                if ((moveX >= 0 && rulerRight < moveX - scaleGap)
                        || width / 2 - rulerRight
                        <= getWhichScalMovex(maxScale + 1) - moveX) {   //去除左右边界

                } else {
                    canvas.drawLine(0, 0, 0, midScaleHeight, midScalePaint);
                    scaleNumPaint.getTextBounds(num1 / scaleGap + minScale + "", 0,
                                                (num1 / scaleGap + minScale + "").length(),
                                                scaleNumRect);
                    canvas.drawText(num1 / scaleCount + minScale + "", -scaleNumRect.width() / 2,
                                    lagScaleHeight
                                            + (rulerHeight - lagScaleHeight) / 2
                                            + scaleNumRect.height(), scaleNumPaint);
                }

            } else {
                if ((moveX >= 0 && rulerRight < moveX)
                        || width / 2 - rulerRight
                        < getWhichScalMovex(maxScale) - moveX) {   //去除左右边界
