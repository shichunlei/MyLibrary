 ...
 * Description:
 * Created by 师春雷
 * Created at 17/11/22 下午12:10
 */
public class AnimNumberTextView extends android.support.v7.widget.AppCompatTextView {
    //高位快
    ...
    protected void onDraw(Canvas canvas) {
        ...
        LogUtils.d("RandomTextView", "draw");
        if (firstIn) {
            firstIn = false;
            super.onDraw(canvas);
            p = getPaint();
            ...
            LogUtils.d("EEEEEEE", "onDraw: " + measuredHeight);
            baseline = (measuredHeight - fontMetrics.bottom + fontMetrics.top) / 2
                    - fontMetrics.top;
    ...
    private void drawNumber(Canvas canvas) {
        ...
        for (int j = 0; j < numLength; j++) {
            ...
            for (int i = 1; i < maxLine; i++) {
                ...
                if (overLine[j] == 0) {
                    drawText(canvas, setBack(arrayListText.get(j), maxLine - i - 1) + "",
                             0 + f0 * j, i * baseline + pianyiliangSum[j], p);
                } else {
                    //定位后画一次就好啦
                    if (overLine[j] == 1) {
                        overLine[j]++;
    ...
    private final Runnable task = new Runnable() {
        ...
        public void run() {
            ...
            if (auto) {
                LogUtils.d("RandomTextView", "" + auto);
