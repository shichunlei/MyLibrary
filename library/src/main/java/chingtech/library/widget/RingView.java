...
public class RingView extends View {
    ...
    private void initAttr(AttributeSet attrs) {
        ...
        defaultColor = a.getColor(R.styleable.RingView_arc_select_color,
                                  ContextCompat.getColor(getContext(), R.color.selectColor));
        selectTextColor = a.getColor(R.styleable.RingView_arcSelectTextColor, Color.WHITE);
        bgColor = a.getColor(R.styleable.RingView_arc_bg_color,
                             ContextCompat.getColor(getContext(), R.color.arc_bg));
        unSelectTextColor = a.getColor(R.styleable.RingView_arcTextColor,
                                       ContextCompat.getColor(getContext(), R.color.arc_text));
            ...
            case MotionEvent.ACTION_MOVE:
              /*  int  disX = (int) (event.getX() - mDownX);
                ...
                if (z >= centerRadius - circleBorder / 2 && z <= centerRadius + circleBorder + 20) {
                    double angle = Math.abs(Math.toDegrees(Math.atan(
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (centerPoint == null) {
            ...
            oval = new RectF(margin + (px - py), margin, maxRadius * 2 - margin + (px - py),
                             maxRadius * 2 - margin);
        ...
        if (!isAnim) {
            canvas.drawArc(oval2, startAngle + perAngle * selectPosition, totalAngle, false,
                           mShadowPaint);
        }
        canvas.drawArc(oval, startAngle + perAngle * selectPosition, totalAngle, false,
                       mCenterCirclePaint);

        for (int i = 0; i < totalSection; i++) {
            if (i == selectPosition) {
                int selectColor = (colorRes == null ? defaultColor :
                        ContextCompat.getColor(getContext(), colorRes[i]));
            ...
            if (TextStrArray != null) {
                ...
                canvas.drawTextOnPath(TextStrArray[i], path, 0,
                                      circleBorder / 2 - mTextHeight / 2 + 6, mIntervalsPaint);
            } else {
                canvas.drawTextOnPath((i + 1) + "", path, 0, circleBorder / 2 - mTextHeight / 2,
                                      mIntervalsPaint);
            }
            startAngle += perAngle;
        }
