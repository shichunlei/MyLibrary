 ...
 * MyLibrary
 * Package chingtech.library.utils
 * Description: 输入框EditText输入银行卡,输入手机,输入身份证格式化的实现
 * Created by 师春雷
 * Created at 17/7/30 上午9:55
 */
public class AddSpaceTextWatcher implements TextWatcher {
    ...
    private int insertSpace(int index, int spaceNumberAfter) {
        switch (spaceType) {
            ...
            case mobilePhoneNumberType:
                if (index == 3 || ((index > 7) && ((index - 3) % (4 * spaceNumberAfter)
                        == spaceNumberAfter))) {
                    buffer.insert(index, ' ');
                    spaceNumberAfter++;
                }
            ...
            case IDCardNumberType:
                if (index == 6 || ((index > 10) && ((index - 6) % (4 * spaceNumberAfter)
                        == spaceNumberAfter))) {
                    buffer.insert(index, ' ');
                    spaceNumberAfter++;
                }
    ...
    public boolean setText(CharSequence charSequence) {
        if (editText != null
                ...
                && computeSpaceCount(charSequence) <= maxLenght) {
            isSetText = true;
    ...
    public enum SpaceType {
        defaultType, //默认类型
        bankCardNumberType, //银行卡类型
        mobilePhoneNumberType, // 手机号类型
        IDCardNumberType;//身份证类型
    }
}
