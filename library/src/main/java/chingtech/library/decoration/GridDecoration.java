...
public abstract class GridDecoration extends NormalDecoration {
    ...
    public GridDecoration(int itemTotalCount, int span) {
        ...
        for (int pos = 0; pos < itemTotalCount; pos++) {
            ...
            if (!headerPaddingSet.contains(pos) && (pos == 0 || !curHeaderName.equals(
                    getRealHeaderName(pos - 1)))) {//如果是分组头部
    ...
    //用来记录每个头部的paddintTop信息
    private Set<Integer>         groupHeadPos     = new TreeSet<>();
    //记录每个分组第一个头部的pos【用于计算当前组最后一个item的span】
        ...
        if (lookup == null) {
            lookup = new GridLayoutManager.SpanSizeLookup() {//相当于weight
                ...
                public int getSpanSize(int position) {
                    ...
                    if (index >= 0) {
                        returnSpan = headerSpanArray.valueAt(
                                headerSpanArray.indexOfKey(position));   //设置itemView PaddingTop的距离
            ...
            final GridLayoutManager gridLayoutManager
                    = (GridLayoutManager) parent.getLayoutManager();
