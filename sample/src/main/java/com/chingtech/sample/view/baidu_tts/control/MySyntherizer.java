...
import com.chingtech.sample.view.baidu_tts.MainHandlerConstant;
...
public class MySyntherizer implements MainHandlerConstant {
    ...
    protected boolean init(InitConfig config) {
        ...
        if (isMix) {
            mSpeechSynthesizer.setParam(SpeechSynthesizer.PARAM_TTS_TEXT_MODEL_FILE,
                                        offlineResource.getTextFilename()); // 文本模型文件路径 (离线引擎使用)
            mSpeechSynthesizer.setParam(SpeechSynthesizer.PARAM_TTS_SPEECH_MODEL_FILE,
                                        offlineResource.getModelFilename());  // 声学模型文件路径 (离线引擎使用)
            sendToUiThread("离线资源路径："
    ...
    public int loadModel(String voiceType) {
        ...
        try {
            ...
            res = mSpeechSynthesizer.loadModel(offlineResource.getModelFilename(),
                                               offlineResource.getTextFilename());
        } catch (IOException e) {
